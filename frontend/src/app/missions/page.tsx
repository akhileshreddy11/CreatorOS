...
function browserMediaUrl(media?: { media_type?: string; media_path?: string; media_url?: string }) {
  if (!media || media.media_type !== "video/mp4") return null;
  if (media.media_url) return media.media_url.startsWith("http") ? media.media_url : `${API_URL}${media.media_url}`;
  return mediaUrl(media.media_path);
}
...
export default function MissionsPage() {
  ...
  useEffect(() => {
    const queryJobId = new URLSearchParams(window.location.search).get("job_id")?.trim();
    const stored = window.localStorage.getItem("creatoros:lastMissionJob");
    const initialJobId = queryJobId || stored;
    if (initialJobId) { setJobId(initialJobId); setDiscovering(false); }
    else void discoverLatest();
  }, []);
